...
@jwt_required()
def listar_despesas():
    """Lista despesas, opcionalmente filtradas por mês."""
    try:
        ...
        logger.info("GET /api/despesas - Encontrados %s registros", len(despesas))
        return _success_response(despesas)

    except Exception as e:
        logger.error("ERRO GET /api/despesas: %s", e, exc_info=True)
...
@jwt_required()
def criar_despesa():
    """Cria uma nova despesa (com validação de data futura e enum)."""
    try:
        ...
        required = [
            ...
            'tipo_pg', 'colaborador_id', 'categoria',
        ]
        missing = [f for f in required if not data.get(f)]
        if missing:
            return _error_response(
                f'Campos obrigatórios faltando: {missing}', 'MISSING_FIELDS'
            )

        try:
            data_compra = datetime.strptime(
                data['data_compra'].split('T')[0], '%Y-%m-%d'
            ).date()
            ...
            return _error_response(
                'Dados inválidos (data, valor ou colaborador_id)', 'INVALID_DATA'
            )

        # NOVA VALIDAÇÃO: data_compra não pode ser no futuro
        if data_compra > date.today():
            return _error_response(
                'Data da compra não pode ser no futuro', 'FUTURE_DATE'
            )
        ...
        # Get collaborator's closing day + INSERT (commit manual)
        with get_db_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute(
                    "SELECT dia_fechamento FROM colaborador WHERE id = %s",
                    (colab_id,),
                )
                colab = cur.fetchone()
                if not colab:
                    return _error_response(
                        'Colaborador não encontrado', 'COLLABORATOR_NOT_FOUND', 404
                    )

                mes_vigente = calcular_mes_vigente(
                    data_compra, tipo_pg, colab['dia_fechamento']
                )

                cur.execute(
                    """INSERT INTO despesa (
                        data_compra, mes_vigente, descricao, valor,
                        tipo_pg, colaborador_id, categoria
                    ) VALUES (%s, %s, %s, %s, %s, %s, %s) RETURNING id""",
                    (data_compra, mes_vigente, data['descricao'], valor,
                     tipo_pg, colab_id, categoria),
                )
                despesa_id = cur.fetchone()['id']
                conn.commit()   # ✅ persiste o INSERT

        logger.info("Despesa criada: id=%s, mes=%s", despesa_id, mes_vigente)
        ...
        logger.error("ERRO POST /api/despesas: %s", e, exc_info=True)
...
def despesa_por_id(id: int):
    """Atualiza ou exclui uma despesa pelo ID."""
    try:
        with get_db_connection() as conn:
            if request.method == 'PUT':
                ...
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    ...
                    cur.execute(
                        """UPDATE despesa
                           SET data_compra=%s, mes_vigente=%s, descricao=%s,
                               valor=%s, tipo_pg=%s, colaborador_id=%s, categoria=%s
                           WHERE id=%s""",
                        (data_compra, mes_vigente, data['descricao'], valor,
        ...
        logger.error("Erro em despesa_por_id: %s", e)

...
def validar_renda_data(data: dict) -> list:
    """Valida os campos de uma renda mensal (colaborador_id, mes_ano, valor)."""
    errors = []
...
@jwt_required()
def rendas():
    """Lista rendas mensais (GET) ou registra/atualiza uma renda (POST/upsert)."""
    try:
        if request.method == 'GET':
            ...
            with get_db_connection() as conn:
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    ...
                    # Decimal values are preserved from database
                    return _success_response(cur.fetchall())

        # POST (upsert) — get_db_cursor comita automaticamente
        ...
        with get_db_cursor() as cur:
            cur.execute(
                "SELECT id FROM colaborador WHERE id = %s",
                (data['colaborador_id'],),
            )
            if not cur.fetchone():
                return _error_response(
        ...
        logger.error("Erro em /rendas: %s", e)
        return _error_response(
            "Erro interno no processamento de rendas", 'OPERATION_FAILED', 500
        )


@rendas_bp.route('/rendas/<int:id>', methods=['PUT', 'DELETE'])
@jwt_required()
def renda_id(id: int):  # pylint: disable=redefined-builtin
    ...
    try:
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                if request.method == 'PUT':
                    ...
                    try:
                        ...
                        if valor <= Decimal('0'):
                            return _error_response(
                                "Valor deve ser um número positivo", 'INVALID_VALUE'
                            )
                    except (InvalidOperation, TypeError):
                        return _error_response(
                            "Valor deve ser um número válido", 'INVALID_VALUE'
                        )
        ...
        logger.error("Erro em /rendas/%s: %s", id, e)
